...
// Configuraci�n de la velocidad para FCY = 1MHz (El proyecto esta en esta config.)
#define FCY 1000000
#define BAUDRATE 19200
...
void Init_UART2(void){
    U2MODEbits.UARTEN = 0; // Se apaga el m�dulo para configurarlo
    
    /* HAY QUE VER BIEN ESTO
    // Configuraci�n de la trama
    ...
    */ 
	...
	//U2BRG = BRGVAL;	// Se carga el valor del c�lculo de baudios
	IPC7 = 0x4400; // Se configura la prioridad de interrupci�n

	IFS1bits.U2RXIF = 0; // Se limpia la bandera de recepci�n
	IEC1bits.U2RXIE = 1; // Se prende la interrupci�n de recepci�n

	U2MODEbits.UARTEN = 1; // Se prende el m�dulo UART
	U2STAbits.UTXEN = 1; // Empieza a transmitir. Se dispara el Flag TXIF

	IFS1bits.U2TXIF = 0; // Se limpia la bandera de transmisi�n
    IEC1bits.U2TXIE = 1; // Se prende la interrupci�n de transmisi�n
}
